...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/print_setting.py" "${KLIPPER_PATH}/klippy/extras/print_setting.py"
    echo "[OK]"
}
...
# Add updater for print_setting to moonraker.conf
add_updater()
{
    ...
    if [ "${update_section}" -eq 0 ]; then
        ...
        echo -e "[update_manager print_setting] already exists in moonraker.conf [SKIPPED]"
    fi
}
...
uninstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/print_setting.py" ]; then
        echo -n "Uninstalling... "
        rm -f "${KLIPPER_PATH}/klippy/extras/print_setting.py"
        echo "[OK]"
        echo "You can now remove the [update_manager print_setting] section in your moonraker.conf and delete this directory. Also remove all [print_setting] configurations from your Klipper configuration."
    else
        echo "print_setting.py not found in \"${KLIPPER_PATH}/klippy/extras/\". Is it installed?"
        echo "[FAILED]"
    fi
}
